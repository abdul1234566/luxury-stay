...
.get("/roomtypes/allroomtype", RoomTypeController.getAllRoomsType)
//users routes
...
router.get('/reservations/latest/:guestId', async (req, res) => {
  const guestId = req.params.guestId;
  const reservation = await Reservation.findOne({ guest: guestId }).sort({ createdAt: -1 }).populate('room');
