...
export default function MaintenanceRequestsTablePage() {
  ...
  return (
    <ProtectedRoute allowedRoles={['guest', 'housekeeping', 'maintenance', 'admin', 'manager']}>
      <section className="!pt-44 pb-20 min-h-screen bg-[#F5F0E1] dark:bg-[#1E1B18] text-[#3B2F2F] dark:text-[#F2E9E1]">
        <div className="container mx-auto max-w-6xl px-5 2xl:px-0">
          <h1 className="text-3xl font-bold mb-12 text-center">Maintenance Requests</h1>
              ...
              {requests.map((req: any) => (
                <div
                  ...
                  className="bg-gradient-to-br from-[#F0E3D6] to-[#DCCAB0] dark:from-[#3A352F] dark:to-[#5B4E43]
                             rounded-3xl shadow-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 transition hover:shadow-3xl"
                >
                  <div className="flex-1">
                    <h3 className="text-xl font-semibold mb-1">{req.room?.name || "Unknown Room"}</h3>
                    <p className="text-sm mb-1"><span className="font-medium">Issue:</span> {req.description}</p>
                    <p className="text-sm mb-1"><span className="font-medium">Type:</span> {req.issueType}</p>
                    <p className="text-sm mb-1"><span className="font-medium">Urgency:</span> {req.urgency}</p>
                    <p className="text-sm mb-1"><span className="font-medium">Location:</span> {req.location}</p>
                    <p className="text-sm mb-1"><span className="font-medium">Allow Access:</span> {req.allowAccess ? "Yes" : "No"}</p>
                    <p className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${getStatusClasses(req.status)}`}>{req.status}</p>
                    <p className="text-xs mt-2"><span className="font-medium">Reported By:</span> {req.reportedBy?.name || req.reportedBy || "-"}</p>
                    <p className="text-xs"><span className="font-medium">Created At:</span> {new Date(req.createdAt).toLocaleString()}</p>
                  </div>
                  ...
                  <div className="flex flex-col md:items-end gap-2 mt-4 md:mt-0">
                      ...
                      assigningId === req._id ? (
                        <div className="flex flex-col gap-2">
                          <select
                            value={selectedUser}
                            onChange={e => setSelectedUser(e.target.value)}
                            className="border rounded px-2 py-1 text-dark dark:text-white bg-white dark:bg-gray-800"
                          >
                            <option value="">Select user</option>
                            {maintenanceUsers.map(user => (
                              <option key={user._id} value={user._id}>{user.name}</option>
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-3 py-1 bg-green-600 text-white rounded font-medium"
                              disabled={!selectedUser}
                            >
                              Assign
                            </button>
                            <button
                              onClick={() => setAssigningId(null)}
                              className="px-3 py-1 bg-gray-300 text-dark rounded font-medium dark:bg-gray-700 dark:text-white"
                            >
                              Cancel
                            </button>
                          </div>
                    ...
                    {(userRole === 'guest') && (
                      <div className="text-sm">
                        {req.assignedTo ? `Assigned to: ${req.assignedTo?.name}` : "Not assigned to maintenance yet"}
                        <div>Status: {req.status}</div>
                      </div>
                    )}
                  </div>
