...
export default function InvoicePage() {
  ...
  if (!session?.user) {
    return (
      <div className="min-h-screen flex items-center justify-center text-lg font-semibold text-[#3B2F2F] dark:text-[#F2E9E1]">
        Please sign in to view the invoice.
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center text-lg text-[#3B2F2F] dark:text-[#F2E9E1]">
        Loading invoice...
      </div>
    );
  }
  ...
  return (
    <section className="!pt-44 pb-20 min-h-screen bg-[#F5F0E1] text-[#3B2F2F] dark:bg-[#1E1B18] dark:text-[#F2E9E1] transition-colors">
      <div className="container mx-auto max-w-4xl px-5 2xl:px-0 Hotel ">

        {/* Header */}
        <div className="flex justify-between items-center mb-8 ">
          <h1 className="text-3xl font-bold">{`Invoice - ${reservation.reservationId}`}</h1>
          <div className="flex gap-4">
            <button
              onClick={downloadPDF}
              disabled={downloading}
              className="px-6 py-3 bg-[#DCCAB0] text-[#3B2F2F] rounded-lg font-semibold hover:opacity-90 transition-opacity dark:bg-[#A78256] dark:text-[#FFF5E1] flex items-center gap-2"
            >
              {downloading ? "Generating PDF..." : "Download PDF"}
            </button>
            <Link
              href="/reservation-table"
              className="px-6 py-3 bg-[#CBB292] text-[#3B2F2F] rounded-lg font-semibold hover:opacity-90 transition-opacity dark:bg-[#A78256] dark:text-[#FFF5E1]"
            >
              Back to Reservations
            </Link>
          </div>
        ...
        <div 
          id="invoice-content" 
          className="bg-gradient-to-br from-[#F0E3D6] to-[#DCCAB0] dark:from-[#3A352F] dark:to-[#5B4E43] rounded-2xl shadow-2xl p-8"
        >
          {reservation.invoiceHtml ? (
            <div 
              dangerouslySetInnerHTML={{ __html: reservation.invoiceHtml }}
        ...
        {/* Reservation Details */}
        <div 
          className="mt-8 bg-gradient-to-br from-[#F0E3D6] to-[#DCCAB0] dark:from-[#3A352F] dark:to-[#5B4E43] rounded-2xl shadow-2xl p-6"
        >
          <h3 className="text-xl font-bold mb-4">Reservation Details</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h4 className="font-semibold mb-2">Room Information</h4>
              <p><span className="font-medium">Room:</span> {reservation.room?.name || "N/A"}</p>
              <p><span className="font-medium">Type:</span> {reservation.room?.roomType || "N/A"}</p>
            </div>
            <div>
              <h4 className="font-semibold mb-2">Stay Details</h4>
              <p><span className="font-medium">Check-in:</span> {new Date(reservation.checkin).toLocaleDateString()}</p>
              <p><span className="font-medium">Check-out:</span> {new Date(reservation.checkout).toLocaleDateString()}</p>
              <p><span className="font-medium">Guests:</span> {reservation.guests}</p>
            </div>
            <div>
              <h4 className="font-semibold mb-2">Guest Information</h4>
              <p><span className="font-medium">Email:</span> {reservation.guestEmail}</p>
              <p><span className="font-medium">Phone:</span> {reservation.guestPhone}</p>
            </div>
            <div>
              <h4 className="font-semibold mb-2">Status</h4>
              <p><span className="font-medium">Status:</span> {reservation.status}</p>
              {reservation.price && <p><span className="font-medium">Total Price:</span> ${reservation.price}</p>}
            </div>
          </div>
        </div>
      </div>
